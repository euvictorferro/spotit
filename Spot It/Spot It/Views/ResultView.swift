...
struct ResultView: View {
    ...
    private var raritySection: some View {
        VStack(alignment: .leading, spacing: Theme.Spacing.sm) {
            Text(rarityLabel)
                .font(.headline)
                .foregroundStyle(Theme.rarityColor(carInfo.raridade ?? 1))

            GeometryReader { geo in
                let progress = CGFloat(carInfo.raridade ?? 1) / 10
                ZStack(alignment: .leading) {
                    Capsule().fill(Color(.tertiarySystemFill)).frame(height: 6)
                    Capsule()
                        .fill(
                            LinearGradient(colors: [.gray, .blue, .purple, .yellow], startPoint: .leading, endPoint: .trailing)
                        )
                        .frame(width: geo.size.width * progress, height: 6)
                    Circle()
                        .fill(Theme.rarityColor(carInfo.raridade ?? 1))
