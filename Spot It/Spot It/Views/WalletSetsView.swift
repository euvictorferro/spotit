...
struct WalletSetsView: View {
    ...
    private var sets: [(brand: String, info: CarBrandInfo, count: Int, totalValue: Double)] {
        let grouped = Dictionary(grouping: items) { CarBrandInfo.brand(for: $0.modelo) }
            .compactMap { brand, items -> (String, CarBrandInfo, Int, Double)? in
                guard let info = CarBrandInfo.table[brand] else { return nil }
                let total = items.reduce(0) { $0 + $1.valorEstimadoUsd }
                return (brand, info, items.count, total)
            }

        switch sort {
        case .mostComplete:
            return grouped.sorted { Double($0.2) / Double($0.1.knownModels) > Double($1.2) / Double($1.1.knownModels) }
        case .leastComplete:
            return grouped.sorted { Double($0.2) / Double($0.1.knownModels) < Double($1.2) / Double($1.1.knownModels) }
        case .valueHighToLow:
        ...
        VStack(alignment: .leading, spacing: Theme.Spacing.md) {
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: Theme.Spacing.md) {
                ForEach(sets, id: \.brand) { set in
                    SetCard(brand: set.brand, info: set.info, count: set.count, totalValue: set.totalValue)
                }
            }
        }
    }
